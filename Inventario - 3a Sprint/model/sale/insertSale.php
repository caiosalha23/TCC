	...
	if(isset($_POST['saleDetailsItemNumber'])){
		...
		if(!empty($itemNumber) && isset($customerID) && isset($saleDate) && isset($quantity) && isset($unitPrice)){
			...
			if(filter_var($customerID, FILTER_VALIDATE_INT) === 0 || filter_var($customerID, FILTER_VALIDATE_INT)){
				...
				echo '<div class="alert alert-danger"><button type="button" class="close" data-dismiss="alert">&times;</button>Insira um ID do cliente válido</div>';
				exit();
			}
			...
			if($unitPrice == ''){ 
				echo '<div class="alert alert-danger"><button type="button" class="close" data-dismiss="alert">&times;</button>Por favor, insira o preço unitário.</div>';
				exit();
			}
			...
			if(!empty($discount)){
				if(filter_var($discount, FILTER_VALIDATE_FLOAT) === false){
					...
					Insira um valor de desconto válido</div>';
					exit();
				}
			}
			...
			if($stockStatement->rowCount() > 0){
				...
				if($currentQuantityInItemsTable <= 0) {
					...
					echo '<div class="alert alert-danger"><button type="button" class="close" data-dismiss="alert">&times;</button>
					O estoque está vazio. Portanto, não pode fazer uma venda. Selecione um item diferente.</div>';
					exit();
				} elseif ($currentQuantityInItemsTable < $quantity) {
					// Requested sale quantity is higher than available item quantity. Hence abort 
					echo '<div class="alert alert-danger"><button type="button" class="close" data-dismiss="alert">&times;</button>Não há estoque suficiente disponível para esta venda. Portanto, não pode fazer uma venda. Selecione um item diferente.</div>';
					exit();
				}
				else {
					...
					if($customerStatement->rowCount() > 0){
						...
						Detalhes da venda adicionados ao banco de dados e estoques atualizados.</div>';
						exit();
						
					} else {
						echo '<div class="alert alert-danger"><button type="button" class="close" data-dismiss="alert">&times;</button>Cliente não existe.</div>';
						exit();
					}
				}
				...
				echo '<div class="alert alert-success"><button type="button" class="close" data-dismiss="alert">&times;</button>
				O item não existe no banco de dados.</div>';
				exit();
			}
