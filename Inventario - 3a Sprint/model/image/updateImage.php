<?php
	...
	if(isset($_POST['itemImageItemNumber'])){
		...
		if(!empty($itemImageItemNumber)){
				...
				if($itemNumberStatement->rowCount() > 0){
					...
					if(in_array($extension, $allowedTypes)){
						...
						if(move_uploaded_file($_FILES['itemImageFile']['tmp_name'], $targetPath)){
							...
							echo '<div class="alert alert-success"><button type="button" class="close" data-dismiss="alert">&times;</button>Imagem atualizada com sucesso.</div>';
							exit();
							
						} else {
							echo '<div class="alert alert-danger"><button type="button" class="close" data-dismiss="alert">&times;</button>Não é possível fazer upload da imagem.</div>';
							exit();
						}
						
					} else {
					// Image type is not allowed
					echo '<div class="alert alert-danger"><button type="button" class="close" data-dismiss="alert">&times;</button>Tipo de imagem não é permitido. Selecione uma imagem válida.</div>';
					exit();
					}
				}
			...
			echo '<div class="alert alert-danger"><button type="button" class="close" data-dismiss="alert">&times;</button>Selecione o número do item</div>';
			exit();
		}
