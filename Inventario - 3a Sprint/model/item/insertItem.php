	...
	if(isset($_POST['itemDetailsItemNumber'])){
		...
		if(!empty($itemNumber) && !empty($itemName) && isset($quantity) && isset($unitPrice)){
			...
			if(filter_var($unitPrice, FILTER_VALIDATE_FLOAT) === 0.0 || filter_var($unitPrice, FILTER_VALIDATE_FLOAT)){
				...
				Insira um número válido para o preço por unidade</div>';
				exit();
			}
			...
			if(!empty($discount)){
				if(filter_var($discount, FILTER_VALIDATE_FLOAT) === false){
					...
					echo '<div class="alert alert-danger"><button type="button" class="close" data-dismiss="alert">&times;</button>Insira um valor de desconto válido</div>';
					exit();
				}
			}
			...
			if($stockStatement->rowCount() > 0){
				...
				echo '<div class="alert alert-danger"><button type="button" class="close" data-dismiss="alert">&times;</button>
				O item já existe no banco de dados. Clique no botão <strong>Atualizar</strong> para atualizar os detalhes. Ou use um número de item diferente.</div>';
				exit();
			} else {
