	...
	if(isset($_POST['customerDetailsCustomerID'])) {
		...
		if(isset($customerDetailsCustomerFullName) && isset($customerDetailsCustomerMobile) && isset($customerDetailsCustomerAddress)) {
			...
			if(filter_var($customerDetailsCustomerMobile, FILTER_VALIDATE_INT) === 0 || filter_var($customerDetailsCustomerMobile, FILTER_VALIDATE_INT)) {
				...
				echo '<div class="alert alert-danger"><button type="button" class="close" data-dismiss="alert">&times;</button>Insira um número de telemóvel válido</div>';
				exit();
			}
			...
			if(empty($customerDetailsCustomerID)){
				echo '<div class="alert alert-danger"><button type="button" class="close" data-dismiss="alert">&times;</button>Insira o ID do Cliente para atualizar esse cliente</div>';
				exit();
			}
			...
			if(!empty($customerDetailsCustomerPhone2)){
				if(filter_var($customerDetailsCustomerPhone2, FILTER_VALIDATE_INT) === 0 || filter_var($customerDetailsCustomerPhone2, FILTER_VALIDATE_INT)) {
					...
					echo '<div class="alert alert-danger"><button type="button" class="close" data-dismiss="alert">&times;</button>Insira um número válido para o número de telefone</div>';
					exit();
				}
			}
			...
			if(!empty($customerDetailsCustomerEmail)) {
				if (filter_var($customerDetailsCustomerEmail, FILTER_VALIDATE_EMAIL) === false) {
					// Email is not valid
					echo '<div class="alert alert-danger"><button type="button" class="close" data-dismiss="alert">&times;</button>Insira um email válido</div>';
					exit();
				}
			}
			...
			if($customerIDSelectStatement->rowCount() > 0) {
				...
				echo '<div class="alert alert-success"><button type="button" class="close" data-dismiss="alert">&times;</button>Detalhes do cliente atualizados.</div>';
				exit();
			} else {
				// CustomerID is not in DB. Therefore, stop the update and quit
				echo '<div class="alert alert-danger"><button type="button" class="close" data-dismiss="alert">&times;</button>ID do Cliente não existe no banco de dados. Portanto, não é possível atualizar.</div>';
				exit();
			}
