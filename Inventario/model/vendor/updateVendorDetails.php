		...
		if(!empty($vendorDetailsVendorID)){
			...
			if(!empty($vendorDetailsVendorFullName) && !empty($vendorDetailsVendorMobile) && !empty($vendorDetailsVendorAddress)) {
				...
				if(filter_var($vendorDetailsVendorMobile, FILTER_VALIDATE_INT) === 0 || filter_var($vendorDetailsVendorMobile, FILTER_VALIDATE_INT)) {
					...
					Por favor insira um número de telemóvel válido</div>';
					exit();
				}
				...
				if(empty($vendorDetailsVendorID)){
					echo '<div class="alert alert-danger"><button type="button" class="close" data-dismiss="alert">&times;</button>
					Insira o ID do fornecedor para atualizar esse fornecedor. Você pode encontrar o ID do fornecedor usando a guia Pesquisar</div>';
					exit();
				}
				...
				if(isset($vendorDetailsVendorPhone2)){
					if(filter_var($vendorDetailsVendorPhone2, FILTER_VALIDATE_INT) === 0 || filter_var($vendorDetailsVendorPhone2, FILTER_VALIDATE_INT)) {
						...
						echo '<div class="alert alert-danger"><button type="button" class="close" data-dismiss="alert">&times;</button>
						Insira um número válido para o número de telefone.</div>';
						exit();
					}
				}
				...
				if(!empty($vendorDetailsVendorEmail)) {
					if (filter_var($vendorDetailsVendorEmail, FILTER_VALIDATE_EMAIL) === false) {
						// Email is not valid
						echo '<div class="alert alert-danger"><button type="button" class="close" data-dismiss="alert">&times;</button>Por favor digite um email válido</div>';
						exit();
					}
				}
				...
				if($vendorIDSelectStatement->rowCount() > 0) {
					...
					echo '<div class="alert alert-success"><button type="button" class="close" data-dismiss="alert">&times;</button>Detalhes do fornecedor atualizados.</div>';
					exit();
				} else {
					// vendorID is not in DB. Therefore, stop the update and quit
					echo '<div class="alert alert-danger"><button type="button" class="close" data-dismiss="alert">&times;</button>
					O ID do fornecedor não existe no banco de dados. Portanto, a atualização não é possível.</div>';
					exit();
				}
