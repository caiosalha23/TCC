	...
	if(isset($_POST['saleDetailsSaleID'])){
		...
		if(isset($saleDetailsItemNumber) && isset($saleDetailsSaleDate) && isset($saleDetailsQuantity) && isset($saleDetailsUnitPrice) && isset($saleDetailsCustomerID)){
			...
			if(filter_var($saleDetailsQuantity, FILTER_VALIDATE_INT) === 0 || filter_var($saleDetailsQuantity, FILTER_VALIDATE_INT)){
				...
				echo '<div class="alert alert-danger"><button type="button" class="close" data-dismiss="alert">&times;</button>Insira um número válido para Quantidade.</div>';
				exit();
			}
			...
			if(filter_var($saleDetailsUnitPrice, FILTER_VALIDATE_FLOAT) === 0.0 || filter_var($saleDetailsUnitPrice, FILTER_VALIDATE_FLOAT)){
				...
				echo '<div class="alert alert-danger"><button type="button" class="close" data-dismiss="alert">&times;</button>Insira um número válido para Preço por Unidade.</div>';
				exit();
			}
			...
			if($saleDetailsDiscount !== ''){
				...
				// Valid discount
					...
					echo '<div class="alert alert-danger"><button type="button" class="close" data-dismiss="alert">&times;</button>Insira um número válido para Desconto.</div>';
					exit();
				}
			}
			...
			if($saleDetailsSaleID == ''){ 
				echo '<div class="alert alert-danger"><button type="button" class="close" data-dismiss="alert">&times;</button>Insira um ID de venda.</div>';
				exit();
			}
			
			// Check if customerID is empty
			if($saleDetailsCustomerID == ''){ 
				echo '<div class="alert alert-danger"><button type="button" class="close" data-dismiss="alert">&times;</button>Insira um ID do cliente.</div>';
				exit();
			}
			...
			if($saleDetailsQuantity == ''){ 
				echo '<div class="alert alert-danger"><button type="button" class="close" data-dismiss="alert">&times;</button>Por favor insira a quantidade.</div>';
				exit();
			}
			...
			if($customerIDStatement->rowCount() < 1){
				...
				echo '<div class="alert alert-danger"><button type="button" class="close" data-dismiss="alert">&times;</button>O ID do cliente não existe no banco de dados. Insira um ID de cliente válido.</div>';
				exit();
			} else {
				$row = $customerIDStatement->fetch(PDO::FETCH_ASSOC);
				$customerID = $row['customerID'];
			...
			if($originalSaleQuantityStatement->rowCount() > 0){
				...
				if($originalOrderItemNumber !== $saleDetailsItemNumber) {
					...
					if($newItemCurrentStockStatement->rowCount() < 1){
						...
						echo '<div class="alert alert-danger"><button type="button" class="close" data-dismiss="alert">&times;</button>O número do item não existe no banco de dados. Se você deseja atualizar este item, adicione-o primeiro ao banco de dados.</div>';
						exit();
					}
					...
					if($stockStatement->rowCount() > 0){
						...
						O item não existe no banco de dados. Portanto, primeiro insira este item no banco de dados usando a guia <strong>Item</strong>.</div>';
						exit();
					}	
				...
				echo '<div class="alert alert-danger"><button type="button" class="close" data-dismiss="alert">&times;</button>
				Os detalhes da venda não existem no banco de dados para o ID de venda fornecido. Portanto, não é possível atualizar.</div>';
				exit();
