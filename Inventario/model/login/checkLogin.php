<?php
	...
	if(isset($_POST['loginUsername'])){
		...
		if(!empty($loginUsername) && !empty($loginUsername)){
			...
			if($loginUsername == ''){
				echo '<div class="alert alert-danger"><button type="button" class="close" data-dismiss="alert">&times;</button>Por favor, digite o nome de usuário</div>';
				exit();
			}
			...
			if($checkUserStatement->rowCount() > 0){
				...
				echo '<div class="alert alert-danger"><button type="button" class="close" data-dismiss="alert">&times;</button>Nome de usuário ou senha incorretos</div>';
				exit();
			}
			...
			Por favor, digite usuário e senha</div>';
			exit();
		}
	}
?>