	...
	if(isset($_POST['resetPasswordUsername'])){
		...
		if(!empty($resetPasswordUsername) && !empty($resetPasswordPassword1) && !empty($resetPasswordPassword2)){
			...
			if($usernameCheckingStatement->rowCount() < 1){
				...
				if($resetPasswordPassword1 !== $resetPasswordPassword2){
					echo '<div class="alert alert-danger"><button type="button" class="close" data-dismiss="alert">&times;</button>As senhas não coincidem.</div>';
					exit();
				} else {
					...
					Redefinição de senha concluída. Por favor, faça o login usando sua nova senha.</div>';
					exit();
				}
			}
		} else {
