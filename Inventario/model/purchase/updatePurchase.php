	...
	if(isset($_POST['purchaseDetailsPurchaseID'])){
		...
		if(isset($purchaseDetailsItemNumber) && isset($purchaseDetailsPurchaseDate) && isset($purchaseDetailsQuantity) && isset($purchaseDetailsUnitPrice)){
			...
			if(filter_var($purchaseDetailsUnitPrice, FILTER_VALIDATE_FLOAT) === 0.0 || filter_var($purchaseDetailsUnitPrice, FILTER_VALIDATE_FLOAT)){
				...
				echo '<div class="alert alert-danger"><button type="button" class="close" data-dismiss="alert">&times;</button>Insira um número válido para o preço unitário.</div>';
				exit();
			}
			
			// Check if purchaseID is empty
			if($purchaseDetailsPurchaseID == ''){ 
				echo '<div class="alert alert-danger"><button type="button" class="close" data-dismiss="alert">&times;</button>Insira um ID da compra.</div>';
				exit();
			}
			
			// Check if itemNumber is empty
			if($purchaseDetailsItemNumber == ''){ 
				echo '<div class="alert alert-danger"><button type="button" class="close" data-dismiss="alert">&times;</button>Por favor, digite o número do item.</div>';
				exit();
			}
			...
			if($originalPurchaseQuantityStatement->rowCount() > 0){
				...
				if($originalOrderItemNumber !== $purchaseDetailsItemNumber) {
					...
					if($newItemCurrentStockStatement->rowCount() < 1){
						...
						O número do item não existe no banco de dados. Se você deseja atualizar este item, adicione-o primeiro ao banco de dados.</div>';
						exit();
					}
				...
				echo '<div class="alert alert-danger"><button type="button" class="close" data-dismiss="alert">&times;</button>Os detalhes da compra não existem no banco de dados para o ID de compra fornecido. Portanto, não é possível atualizar.</div>';
				exit();
